describe("FAS Features", () => {
  ...
  it("Allow book owner to search for users by name or email.", () => {
    ...
    cy.get("#connect_book_user_link").contains("Add a user (admin)").click().wait(3000);
    // lookup by email
    cy.get('input[name="searchValue"]').type(Cypress.env('testUser')).wait(3000);
    cy.get('div[class="results"]').contains(Cypress.env('testUser'));
    //  lookup by user full name
    cy.get('input[name="searchValue"]').clear()
    cy.get('input[name="searchValue"]').type(Cypress.env('testUserFullName')).wait(3000);
  ...
  it("Test Mirador fix - multiple mirador instances on the page", () => {
    ...
    // add to page
    cy.get(
      'a[href="https://scalar-dev.fas.harvard.edu/features-test-book/new.edit"]'
    ).click();
